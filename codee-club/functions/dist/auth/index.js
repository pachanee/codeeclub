...
const getUrl = (data, _context) => {
    return (0, nu_connect_1.authorizeUrl)(data.redirectUri);
};
exports.getUrl = getUrl;
const getToken = (data, _context) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    const response = yield (0, nu_connect_1.exchangeAuthCodeForToken)(data.authCode, data.redirectUri);
    if ('error' in response.body) {
        console.error(response.body.error);
        return { error: 'Failed to exchange NU token' };
    }
    const nuToken = response.body;
    const userIdentity = yield (0, nu_connect_1.getUserIdentity)(nuToken);
    let userId;
    let userData;
    try {
    ...
    const { nameFirst, nameLast } = (0, string_cases_1.fixName)(userIdentity.name);
