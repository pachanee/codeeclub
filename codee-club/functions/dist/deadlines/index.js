...
const sendDeadlineReminders = (_context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const deadlines = courses.flatMap(course => (0, object_extensions_1.entries)(course.activities)
        .filter(activityIdAndActivity => activityIdAndActivity[1].status === enums_1.ActivityStatus.Published)
        .flatMap(activityIdAndActivity => (0, object_extensions_1.entries)(activityIdAndActivity[1].deadlines)
    ...
    for (const deadline of deadlines) {
        ...
        if (NOTIFY_AT_DAYS.has(dueInDays))
            yield (0, notifications_1.notify)(deadline.studentIds, (0, notifications_1.notificationDeadlineReminder)(now, deadline.courseSlug, deadline.activitySlug, deadline.activityName, dueInDays));
    }
    return null;
});
